...
export function NotificationItem({ notification }: NotificationItemProps) {
  ...
  const content = (
    ...
    >
      <div className={`mt-0.5 rounded-full p-1.5 ${isUnread ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-500'}`}>
        <Icon className="h-4 w-4" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm text-gray-800">{notification.message}</p>
        <p className="mt-0.5 text-xs text-gray-400">{relativeTime(notification.created)}</p>
      </div>
      {isUnread && <div className="mt-2 h-2 w-2 shrink-0 rounded-full bg-primary-500" />}
    </div>
  );
