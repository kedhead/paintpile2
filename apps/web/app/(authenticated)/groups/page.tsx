...
export default function GroupsPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <h1 className="text-xl font-bold text-foreground mb-1">Discover Groups</h1>
      <p className="text-sm text-muted-foreground mb-6">Find communities to join, or create your own.</p>

      {isLoading ? (
        <div className="text-center text-muted-foreground py-10">Loading groups...</div>
      ) : publicGroups?.length === 0 ? (
        <div className="text-center py-20">
          <Users className="mx-auto h-12 w-12 text-muted-foreground mb-3" />
          <h3 className="text-foreground font-medium mb-1">No groups yet</h3>
          <p className="text-sm text-muted-foreground">Create the first one using the + button!</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {publicGroups?.map((group: RecordModel) => (
            <div
              key={group.id}
              className="rounded-lg border border-border bg-card p-4 hover:border-primary-300 transition-colors"
            >
